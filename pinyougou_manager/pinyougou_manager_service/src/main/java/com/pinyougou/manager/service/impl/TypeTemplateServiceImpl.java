...
public class TypeTemplateServiceImpl implements TypeTemplateService {
	...
	public List<Map> findSpecList(Long id) {
		...
		List<Map> specIdsMap = JSON.parseArray(specIds, Map.class);
		for (Map specIdMap:specIdsMap) {
			//得到规格Id
			Long specId = Long.valueOf(String.valueOf(specIdMap.get("id")));
			TbSpecificationOptionExample example = new TbSpecificationOptionExample();
			example.createCriteria().andSpecIdEqualTo(specId);
			List<TbSpecificationOption> optionList = specificationOptionMapper.selectByExample(example);
			specIdMap.put("options",optionList);
